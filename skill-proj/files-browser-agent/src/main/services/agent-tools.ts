...
export function makeTools(emit: Emit, client: Anthropic, workDir: string) {
  ...
  return [
    betaTool({
      name: 'activate_skill',
      description:
        'Activate one of the available skills by name and return its full instructions.',
      inputSchema: activateSkillSchema,
      run: async ({ name }) => {
        const id = start('activate_skill', { name });

        // מחפשים את הסקיל בזיכרון, לא בדיסק
        const body = SkillsStore.getBody(name);

        if (!body) {
          fail(id, `Skill not found: ${name}`);
          return `Error: Skill "${name}" was not found.`;
        }

        // מחזירים למודל את ההנחיות המלאות
        ok(id, name);
        return body;
      }
    }),
