...
export type LoadedSkill = {//יצירת טיפוס לטעינת הבסיס של הסקילים
  name: string;
  description: string;
  body: string;
  path: string;
...
export const SkillsStore = {
  // כאן נשמור בזיכרון את כל הסקילים שנטענו
  skills: [] as LoadedSkill[],

  // טוען מחדש את הסקילים מהדיסק ושומר אותם בזיכרון
  async reload(): Promise<void> {
    this.skills = await loadSkills();
  },

  // מחזיר קטלוג קצר: רק שם ותיאור, בלי כל ה-body הארוך
  listAvailable(): Pick<LoadedSkill, 'name' | 'description'>[] {
  ...
  // מחפש סקיל לפי שם ומחזיר את התוכן המלא שלו
